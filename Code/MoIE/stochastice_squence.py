...
# -*- coding: utf-8 -*-
'''
@File    :   stochastice_squence.py
@Time    :   2025/10/09 13:40:40
@Author  :   Dunyuan XU
@Version :   0.1
@Desc    :   None
'''
...
def learn_batch(test_loaders, iterations, MoE_trainer, pretrained_model, log_dir, args, sats):
    ...
    all_sites = ['Drishti_GS', 'Magrabia', 'ORIGA', 'REFUGE']
    finetune_dice = []
    print("*"*20+" start adaptating "+"*"*20)
    logging.info("*"*20+" start adaptating "+"*"*20)
    slice_wise = {a:[] for a in all_sites}
    adapt_iter=0
    for batch in test_loaders:
        adapt_iter+=1
        site_name, name, img, gt = batch['site_name'][0], batch['slice_name'], batch['img'].cuda(), batch['gt'].cuda()
        site_index = all_sites.index(site_name)+1
        ...
        slice_wise[site_name].append(avg_dice)
        
        
    for site in all_sites:
        finetune_dice.append(sum(slice_wise[site]) / len(slice_wise[site]))
        
    print("total adaptation iter: ", str(adapt_iter))
    logging.info("total adaptation iter: "+ str(adapt_iter))
    
    txt_dir = os.path.join(log_dir, 'explaination.txt')
    MoE_trainer.output_results(txt_dir)
            
    print('-----> Finetune MEAN test_dice = %.4f' % (sum(finetune_dice) / len(finetune_dice)))
    ...
    logging.info('-----> Finetune MEAN test_dice = %.4f' % (sum(finetune_dice) / len(finetune_dice)))
...
def main():
    ...
    if args.record:
        ...
        cur_dir = os.path.join("./augment/", "random")
        if not os.path.exists(cur_dir):
            os.mkdir(cur_dir)
        if args.network == 'Unet':
            log_dir = os.path.join(cur_dir, "random_ours_seed:"+str(seed)+"_network:"+str(args.network)+"_aug:"+str(args.augment)+"-"+ TIMESTAMP)
        else:
            log_dir = os.path.join(cur_dir, "random_ours_seed:"+str(seed)+"_network:"+str(args.network)+"_aug:"+str(args.augment)+"_experts:"+str(args.num_experts)+"-"+ TIMESTAMP)
    ...
    _, all_test_loader, _ = joint_loader(args.data_npz, sites, args.batch_size)
